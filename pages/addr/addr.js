    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          wx.request({
            method: 'POST',
            data: { addr_id: _this.data.addrs[idx].id },
      ...
      success: function (res) {
        ...
        if (d.result == 1 && d.data && d.data.addressList) {
          ...
          addrList.forEach(function(item, idx){
            var addr = {
              id: item.addr_id, name: item.name, province: item.province ? item.province : '', city: item.city ? item.city : '', 
              region: item.region ? item.region: '', fullAddress : '',
              detail: item.addr, tel: item.mobile, isDefault: (item.def_addr == 1),
            };
