...
Page({
  ...
  bindName: function(e) {
    ...
    this.setData({addr: this.data.addr});
  },
  ...
  saveNewAddr: function() {
    ...
    wx.request({
      method: 'POST',
      data: { def_addr: addr.isDefault, name: addr.name, mobile: addr.tel, province: addr.province, city: addr.city, region: addr.region, 
        zip: addr.zip, addr: addr.address, shipAddressName: addr.shipAddressName },
  ...
  updateAddr: function() {
    ...
    wx.request({
      method: 'POST',
      data: {
        addr_id: addr.id, def_addr: addr.isDefault, name: addr.name, mobile: addr.tel, province: addr.province, city: addr.city, region: addr.region,
        zip: addr.zip, addr: addr.address, shipAddressName: addr.shipAddressName
      },
    ...
    if(addrId) {
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (d && d.result == 1) {
            _this.addr = {
              id: d.data.addr_id, name: d.data.name, province: d.data.province ? d.data.province : '', city: d.data.city ? d.data.city : '',
              region: d.data.region ? d.data.region : '', fullAddress: '', zip: d.data.zip, shipAddressName: d.data.shipAddressName,
              address: d.data.addr, tel: d.data.mobile, isDefault: (d.data.def_addr == 1),
            };
            _this.region = [ d.data.province, d.data.city, d.data.region ];
            _this.setData({ addr: _this.addr, region: _this.region });
          } else {
            wx.showToast({
              icon: "none",
              title: '未获取到地址信息',
