...
Page({
	...
	data: {
		...
		bannerSwiper: {
			...
			images: [{ url: "/images/800x450-2.jpg", id: 100 },
			{ url: "/images/800x504c.jpg", id: 101 },
				{ url: "/images/554x380.jpg", id: 102 }]
		},
		noticeSwiper: {
			...
			success: function (res) {
				...
				if (r && r.result == 1) {
					...
					for (var i = 0; i < r.data.length; i++) {
						...
						imgs.push({ url: r.data[i].atturl, id: id });
					}
					page.data.bannerSwiper.images = imgs;
					page.setData({ bannerSwiper: page.data.bannerSwiper });
				}
			}
		});
	},
		...
		wx.request({
			...
			success: function (res) {
				...
				if (r && r.result == 1) {
					var notices = [];
					for (var i = 0; i < r.data.length; i++) {
						notices.push({ title: r.data[i].title, id: r.data[i].id });
					}
					page.data.noticeSwiper.notices = notices;
					page.setData({ noticeSwiper: page.data.noticeSwiper });
				}
			}
		});
	},
		...
		wx.request({
			...
			success: function (res) {
				...
				if (r && r.result == 1) {
					...
					page.setData({ categoryScroll: page.data.categoryScroll });
				}
			}
		});
	},
		...
		wx.request({
			...
			success: function (res) {
				...
				if (r && r.result == 1) {
					...
					page.setData({ categoryProducts: page.data.categoryProducts });
				}
			}
		});
	},
		...
		wx.getSetting({
			success: res => {
				if (res.authSetting['scope.userInfo']) {
					...
					this.setData({ isShowAuthorize: true });
				}
			}
		});
	},
